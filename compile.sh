...
EXT_VANILLAGENERATOR_PM5_VERSION="abd059fd2ca79888aab3b9c5070d83ceea55fada"
...
function build_libffi {
	write_library libffi "$LIBFFI_VER"
	local libffi_dir="./libffi-$LIBFFI_VER"

	if cant_use_cache "$libffi_dir"; then
		rm -rf "$libffi_dir"
		write_download
		download_file "https://github.com/libffi/libffi/releases/download/v$LIBFFI_VER/libffi-$LIBFFI_VER.tar.gz" "libffi" | tar -zx >> "$DIR/install.log" 2>&1
		echo -n " checking..."
		pushd $libffi_dir >> "$DIR/install.log" 2>&1
		if [ "$DO_STATIC" != "yes" ]; then
			local EXTRA_FLAGS="--enable-shared --disable-static"
		else
			local EXTRA_FLAGS="--enable-shared --enable-static"
		fi
		./configure \
		  --prefix="$INSTALL_DIR" \
		  --libdir="$INSTALL_DIR/lib" \
